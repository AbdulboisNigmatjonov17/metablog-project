            ...
            <Link href={`/post/${item.id}`} className='w-full'>
                <div className='w-full overflow-hidden'>
                    <img
                        ...
                        // width={360}
                        // height={240}
                        className='w-full h-[240px] object-cover rounded-md transition duration-300 ease-in-out hover:scale-125 '
                    />
                </div>
            </Link>
            <div className='w-full h-full flex flex-col justify-between items-start'>
                ...
                <div className='w-full flex items-center justify-between'>
                    <Link href={`/author/${item.user.name}`} className='flex items-center gap-3'>
                        <img src={`${item.user.img}`} alt={`card ${item.id}`} width={36} height={36} className='rounded-full' />
